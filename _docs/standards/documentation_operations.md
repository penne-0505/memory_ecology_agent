...
schema marker は対象文書型に固有である。Intent に `qa_schema`、QA に
`intent_schema`、または未定義の `*_schema` marker を置かない。front matter 内で
同じ key を重複定義せず、validator は後勝ちで解釈せず error とする。
